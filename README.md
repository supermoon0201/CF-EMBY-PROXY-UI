...
| 协议优化 | H1/H2/H3 开关、晚高峰自动降级、媒体 `403` 兼容梯子、`bytes=0-...` 无 `Range` 降级 |
...
这里的“真实 IP”指的是客户端与 Cloudflare 建立连接时被 Cloudflare 识别到的来源 IP。它可以是用户本机的公网出口 IP，也可以是用户前置代理的出口 IP。默认情况下，节点会透传 `X-Real-IP`；当节点切到 `dual` 时，项目还会额外补写 `X-Forwarded-For`。其中 `X-Forwarded-For` 不是完整代理链，而是单个真实 IP。并且注入发生在节点自定义请求头之后，因此节点里新增同名请求头也不能覆盖这两个值。如果个别上游会按真实出口 IP、地区或 ASN 做风控，也可以在节点级改为仅保留 `X-Real-IP`、或直接关闭透传【慎用】。
...
- 非 Emby 原生 `.strm` 会先解析出首个有效 `http/https` URL，再继续沿用现有的外链直连 / 反代决策，而不是一律直接下发
...
- 节点级支持 `realClientIpMode = smart / realip_only / off / dual`，并兼容旧值 `forward / strip / disable`
...
- 真实客户端 IP 透传由 `X-Real-IP` 和 `X-Forwarded-For` 决定；默认情况下节点会透传 `X-Real-IP`，并按模式决定是否同时补写 `X-Forwarded-For`
- 注入发生在节点自定义请求头之后，因此节点里新增同名请求头也不能覆盖 `X-Real-IP` / `X-Forwarded-For`
- `smart` 与 `realip_only` 都会保留 `X-Real-IP`，其中 `smart` 作为默认推荐值，便于兼容大部分按来源 IP 风控的上游
- `dual` 会同时补写 `X-Real-IP` 和 `X-Forwarded-For`；适用于需要最大兼容性的老上游或前置层
- `off` 会强制不透传 `X-Real-IP` 和 `X-Forwarded-For`；适用于个别上游不接受这两个请求头的场景，需谨慎使用

节点编辑面板中的“真实客户端 IP 透传”建议按下面理解：

- `smart`（界面显示为“自动（推荐，仅 X-Real-IP）”）：默认推荐，仅保留 `X-Real-IP`
...
- 当媒体 `GET/HEAD` 或可安全重放的请求首次回源返回 `403` 时，Worker 会在当前已选中的上游目标上执行一次兼容梯子回退，而不是重新遍历全部节点
...
- 只处理普通 `.strm` 文本请求，不处理 Emby 原生 `/emby/videos/.../stream.strm`
- `.strm` 文本会忽略空行和 `#` 注释，并取首个合法的 `http/https` URL
- 解析出的真实 URL 不继承原始播放请求 query，避免把 Emby/Jellyfin 参数错误拼到外部盘链
- 解析成功后仍然继续走当前 `worker.js` 的外链直连 / 强制反代 / `wangpandirect` 决策
- 如果 `.strm` 文本里没有合法 URL，会返回明确的 `400`
...
| `listRemoteCandidateIps` | 读 | 读取远程候选优选 IP | 供 `优选调度` 页面拉取候选 IP，再由浏览器侧完成测速与排序 |
